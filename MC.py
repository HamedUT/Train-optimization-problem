...
def calculate_combined_journey_exact_time(total_distance, delta_t, speed_limit, max_acc, max_braking, m, C_d, A, C, eta, WindSpeed, v_init, max_p, braking_eff, total_time, plot=False):
    ...
    if plot == True:
        print(f"Difference: {closest_difference:.3f} m, braking: {braking_speed_final*3.6:.1f} km/h, Max speed: {speed_limit*3.6:.1f} km/h, acc time: {acceleration_time_final:.1f}, coasting: {coasting_time_final:.1f} s, braking time: {braking_time_final:.1f} s, Total time: {total_time_final:.1f} s")
        ...
        save_velocity_and_power_data("MC_results.csv", distances_final, velocities_final, powers_final, accelerations_final, total_energy=total_energy_final)
        # Return the total distance traveled
    return distances_final,velocities_final, powers_final, braking_speed_final, distance_final, total_time_final
...
for speed_limit in [i * 1 for i in range(0, int(speed_limit_allowed) + 1)]:  # Increment by 0.1 m/s
    distances,velocities, powers, braking_speed_final, distance_final, total_time_final = calculate_combined_journey_exact_time(total_distance, delta_t, speed_limit, max_acc, max_braking, m, C_d, A, C, eta, WindSpeed, v_init, max_p, braking_eff, total_time, plot=False)    
    ...
    if difference < closest_difference:            
        ...
        distances_final,velocities_final, powers_final = distances,velocities, powers
