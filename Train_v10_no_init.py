...
# v10_no_init: Removed the initializer function and its call. The model is now directly initialized in the train function. The results are mixed, sometimes initializer works better (in smaller delta_s) and sometimes the non-initializer works better (in bigger delta_s). in time-limited problems, noinit fails to find a feasible solution.
