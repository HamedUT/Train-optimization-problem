...
def Initializer(rho_1, rho_2, S, V0, delta_t, max_acc, max_p, data, m, C_d, A, C, theta, eta, braking_eff):
    """
    Initialize the model with velocity as decision variable to get feasible starting points
    for the power-based model.
    """
    T = data.keys()
    ...
    if results.solver.termination_condition == pyomo.TerminationCondition.optimal:
        ...
        s_opt = {t: model0.s[t].value for t in T}
    else:
        print(f"Infeasible during Initialization - {results.solver.termination_condition}")
        sys.exit()
    return v_opt, P_opt, s_opt

def train(rho_1, rho_2, S, V0, delta_t, max_acc, max_p, data, m, C_d, A, C, theta, eta, braking_eff, max_p_sub1, max_p_sub2):
    """
    Main optimization model with power as the decision variable.
    """
    v_opt, P_opt, s_opt = Initializer(rho_1, rho_2, S, V0, delta_t, max_acc, max_p, data, m, C_d, A, C, theta, eta, braking_eff)    
    T = data.keys()
    
    model = pyomo.ConcreteModel()

    # Decision Variables - Now power is the primary decision variable
    ...
    model.P = pyomo.Var(T, domain=pyomo.Reals, bounds=(-min_p, max_p), initialize=lambda model0, t: P_opt[t])

    # State Variables
    model.v = pyomo.Var(T, domain=pyomo.NonNegativeReals, bounds=(0, max_v), initialize=lambda model0, t: v_opt[t]) # velocity (m/s)
    model.s = pyomo.Var(T, domain=pyomo.NonNegativeReals, initialize=lambda model0, t: s_opt[t]) # Distance
    model.a = pyomo.Var(T, domain=pyomo.Reals, bounds=(-max_acc, max_acc)) # Acceleration (m/s²)
    ...
    # Define the relationship between power components
    for t in list(T)[1:]:
        # Power balance: P = Pm - Pn
        model.cons.add(model.P[t] == model.Pm[t] - model.Pn[t])
        
        # Calculate acceleration based on power and current state
        model.cons.add(model.a[t] == (model.v[t] - model.v[data[t]['t_prev']])/delta_t)
        
        # Relate velocity to distance change
        model.cons.add(model.v[t] == (model.s[t] - model.s[data[t]['t_prev']]) / delta_t)
        
        # Acceleration limits
        model.cons.add(model.a[t] <= max_acc)
        model.cons.add(model.a[t] >= -max_acc)
        ...
        # Key change: Now we relate power to train dynamics through the Davies equation
        # Power = Force × Velocity, where Force accounts for all resistances and acceleration
        model.cons.add(eta * model.P[t] == (
            (0.5 * 1.225 * C_d * A * model.v[t]**2 +  # Aerodynamic resistance
            C * m * 9.807 +                           # Rolling resistance
            m * 9.807 * theta +                       # Gradient resistance
            m * model.a[t])                           # Inertial force 
            * model.v[t]))                            # Multiply by velocity to get power
    ...
    for t in data.keys():
        ...
        if t == '00:00':
            print('  ', t, ':', f"{v_out:.3f}", 'km/h','  ', model.s[t]()/1000, 'km','  ', model.a[t](), 'm/s²','  ', (model.P[t]()), 'W')
        else:
            a_out = model.a[t]()
            print('  ', t, ':', f"{v_out:.2f}", 'km/h','  ', f"{s_out:.3f}", 'km','  ', f"{a_out:.4f}", 'm/s²','  ', f"{p_out:.6f}", 'MW')
...
def plot_velocity_profile(model, data):
    ...
    for t in data.keys():
        ...
        velocities.append(model.v[t]()*3.6)  # Now in km/h
    ...
    plt.title(f'Train Velocity Profile (S={S/1000}km, O.F.={(model.of())/3600000*total_time:.3f} kWh)')
...
def plot_Pm_and_Pn_profile(model, data):
    ...
    for t in data.keys():
        ...
        # Get acceleration
        if t != '00:00':
            accelerations.append(model.a[t]())
        else:
...
def plot_substation_powers(model, data):
    ...
    ax1.set_title(f'Substations Power Profile (S={S/1000}km, Braking Efficiency={braking_eff:.0f})')
    ...
    ax2.set_title(f'Substations Power Profile (S={S/1000}km, Braking Efficiency={braking_eff:.0f})')
...
if termination_condition == pyomo.TerminationCondition.optimal or termination_condition == pyomo.TerminationCondition.locallyOptimal:
    # save to file
    base_filename = f"t{total_time}_S{S}_power_based"
    ext = ".txt"
    suffix_num = 0
    ...
    with open(filename, 'w') as f:
        ...
        for t in data.keys():
            v_out = 3.6 * model.v[t]()
            if model.P[t]() < 0:
                P_out = braking_eff * model.P[t]() / 1000000 # Added braking_eff when braking
            else:
                P_out = model.P[t]() / 1000000
