...
# Train Parameters
m = 152743 * (1+0.0674) # kg (train weight, but should be variable later)
...
def Initializer(rho_1, rho_2, S, V0, delta_t, max_acc, max_p, data, m, C_d, A, C, theta, eta, braking_eff):
    ...
    for t in list(T)[1:]:
        ...
        model0.cons.add(-0.5 <= (model0.v[t] - model0.v[data[t]['t_prev']])/delta_t)
...
def train(rho_1, rho_2, S, V0, delta_t, max_acc, max_p, data, m, C_d, A, C, theta, eta, braking_eff, max_p_sub1, max_p_sub2):
    ...
    for t in list(T)[1:]:
        ...
        model.cons.add(-.5 <= (model.v[t] - model.v[data[t]['t_prev']])/delta_t)
    ...
    print('Value of O.F. = {:.3f} kWh'.format((model.of()*delta_t)/3600000))
