...
power_limits_1 = []
with open('power_limits_1.txt', 'r') as f:
    for line in f:
        power_limits_1.append(float(line.strip()))

power_limits_2 = []
with open('power_limits_2.txt', 'r') as f:
    for line in f:
        power_limits_2.append(float(line.strip()))
...
def calculate_Pm_per_second(model, data):
    # Create a dictionary to store the sum of Pm for each second
    Pm_per_second = {}
    ...
    for t in data.keys():
        ...
        Pm_value = model.Pm[t]() / 1000000  # Convert Pm from W to MW

        if time not in Pm_per_second:
            Pm_per_second[time] = 0
        Pm_per_second[time] += Pm_value  # Sum Pm values for the same second

    # Calculate the total sum of Pm over all seconds
    total_Pm = sum(Pm_per_second.values())
    total_Pm = total_Pm / len(Pm_per_second)
    print(f"Total Pm over all seconds: {total_Pm:.6f} MW")
    print(f"Total Pm over all seconds: {len(Pm_per_second)} times")
...
if termination_condition == pyomo.TerminationCondition.optimal or termination_condition == pyomo.TerminationCondition.locallyOptimal:
    ...
    calculate_Pm_per_second(model, data)
    plt.show()
else:
