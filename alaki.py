...
def train(distance_remaining, delta_s, max_acc, max_braking, max_p, data, m, C_d, A, C, eta, braking_eff, time_remaining, WindSpeed, v_init, max_p_sub1, max_p_sub2, mu_curve, Consider_electrical_losses, rho, V0):
    ...
    model.of = pyomo.Objective(expr=sum((model.Pm[d] - model.Pn[d] * braking_eff)*(2 * delta_s / (model.v[d] + model.v[d - delta_s])) for d in list(D)[1:]), sense=pyomo.minimize)
...
def plot_Pm_and_Pn_profile_time(model, data, speed_limits=None):
    ...
    ax1.xaxis.grid(True, which='both', linestyle='--', alpha=0.7)
    # Only show grid for ax1 (left y and x axes)
    ax2 = ax1.twinx()
    ...
    plt.grid(False)  # Disable global grid   
...
braking_eff = 0.5  # Regenerative braking efficiency (when converting electrical energy to mechanical energy)
...
total_time = 390 # (sec) From Substation 1 to Substation 2
delta_s = 25  # Distance step in meters

# Time-dependent parameters
max_p_sub1 = 15.5 * 1000000 # W (max power for substation 1)
max_p_sub2 = 15.0 * 1000000 # W (max power for substation 2)
...
speed_limit_csv_path = r"c:\Users\DarbandiH\OneDrive - University of Twente\Postdoc\Python\Train optimization problem\SpeedLimit_alaki.csv"
